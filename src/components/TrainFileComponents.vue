...
<template>
    <div>
        <div class="card">
            ...
            <DataTable ref="dt" :value="trainFileStore.getFileList" dataKey="id" 
                responsiveLayout="scroll"
                :paginator="true" :rows="trainFileStore.getPageSize" :totalRecords="trainFileStore.getTotalRecord"
                ...
                <Column field="status" header="Trạng thái" style="width: 10%">
                    <template #body="slotProps">
                        <span :style="{color: slotProps.data.status === 0 ? 'red' : 'green'}">
                            {{ slotProps.data.status === 0 ? 'Chưa huấn luyện' : 'Đã huấn luyện' }}
                        </span>
                    </template>
                </Column>
                ...
                <Column :exportable="false" style="width: 6%">
                    <template #body="slotProps">
                        <Toast/>
                        <Button v-if="slotProps.data.status == 0" icon="pi pi-arrow-up" outlined rounded class="mr-2" @click="updateFile(slotProps.data)" />
                        <Button v-if="slotProps.data.status == 0" icon="pi pi-trash" outlined rounded severity="danger" @click="confirmDeleteFile(slotProps.data)" />
                    </template>
                </Column>
            </DataTable>
        </div>
        <Dialog v-model:visible="trainDialog" :style="{width: '450px'}" header="Xác nhận huấn luyện" :modal="true">
            <div class="confirmation-content">
                <i class="pi pi-exclamation-triangle mr-3" style="font-size: 2rem" />
                <span v-if="fileTrain">Bạn chắc chắn muốn sử dụng file <b>{{fileTrain.file_name}}</b> để huấn luyện chatbot?
                <b style="color: red">Khi đã huấn luyện thì file sẽ không thể xóa khỏi hệ thống</b></span>
            </div>
            <template #footer>
                <Button label="Không" icon="pi pi-times" text @click="hideTrainDialog"/>
        ...
        <Dialog v-model:visible="deleteDialog" :style="{width: '450px'}" header="Xác nhận xóa file huấn luyện" :modal="true">
            <div class="confirmation-content">
                <i class="pi pi-exclamation-triangle mr-3" style="font-size: 2rem" />
                <span v-if="fileTrain">Bạn chắc chắn muốn xóa file <b>{{fileTrain.file_name}}</b>?</span>
            </div>
            <template #footer>
    ...
    <Dialog
      ...
      class="p-fluid"
    >
        <div class="flex flex-row align-items-center justify-content-end gap-3 py-5">
            ...
            <Button label="Tải lên" icon="pi pi-upload" class="w-10rem mx-auto" @click="onUpload" :disabled="!file"></Button>
